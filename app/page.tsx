            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-6 sm:p-8">
              ...
              <ForestPlot
                rows={[
                  ...
                  { label: "total funding raised (era adj., N=793)", or: 1.33, ciLo: 1.08, ciHi: 1.63, p: 0.007, sig: true },
                ]}
              />
            </div>
          </Reveal>
              ...
              <table className="w-full text-sm font-mono">
                ...
                <tbody>
                  {[
                    ...
                    { dep: "total funding raised, log (era adj., OLS, N=793)", coef: "0.284", se: "0.106", z: "2.68", or: "1.33", ci: "[1.08, 1.63]", p: "0.007", r2: "0.066", verdict: "significant", sig: true },
                  ].map((row) => (
          ...
          <Reveal delay={200}>
            <div className="mt-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6">
              ...
              <p className="text-white/80 text-sm sm:text-base mt-2 leading-relaxed">
                Press coverage doesn&apos;t predict who becomes a unicorn — but it does predict who scales and who raises. Press-covered companies have <strong className="text-emerald-400">23% bigger teams</strong> (p=0.009) and are <strong className="text-emerald-400">1.46x more likely to reach YC&apos;s &quot;Growth&quot; stage</strong> (p=0.006), both holding after controlling for batch era <em>and</em> industry composition. We also individually researched total funding raised for 930 companies (310 hot, 620 matched not-hot — a dataset that doesn&apos;t exist anywhere else) and found press-covered companies raise <strong className="text-emerald-400">1.43x more capital</strong> (median $3.0M vs $2.1M, p=0.0098 Mann-Whitney, N=792 with usable data). We replicated this by independently re-sampling the not-hot group — the two samples agreed (p=0.73, no sampling artifact) — but a high-confidence-only cut of the newer sample showed an attenuated, non-significant result, so treat the <em>direction</em> as solid and the exact multiplier as a rough estimate. Hype is a real, measurable signal for scale and fundraising — it just doesn&apos;t reliably predict the extreme, rare outcome of hitting $1B+.
              </p>
            </div>
          </Reveal>
